...
@Component({
  ...
  templateUrl: './ReporteInsumosAProyecto.component.html',
  styleUrls: ['./ReporteInsumosAProyecto.component.scss'],
  providers: [MessageService] 
})
export class ReporteInsumosAProyectoComponent implements OnInit {
...
generateReport() {
  ...
  this.reporte.GetReporteInsumosAProyecto(fechainicio, fechafin).subscribe(
    (data: ReporteInsumosAProyecto[]) => {
      this.loading = false;
      ...
      const filteredData = data.filter(item =>
        item.cantidadproyecto ||
        item.numerocompraproyecto ||
        item.unidadMedidaoRentaproyecto ||
        item.tipoRentaproyecto ||
        item.ultimoPrecioUnitarioproyecto ||
        item.unidadMedidaproyecto ||
        item.fechaFinproyecto ||
        item.fechaInicioproyecto ||
        item.cotizacionProveedorproyecto ||
        item.cantidadVerificadaproyecto ||
        item.ubicacionesproyecto ||
        item.tipoArticuloproyecto
      );
      ...
      autoTable(doc, {
        ...
        head: [[
          'No',
          'Cantidad Proyecto', 
          'Número Compra Proyecto', 
          'Unidad Medida o Renta Proyecto', 
          'Tipo Renta Proyecto', 
          'Último Precio Unitario Proyecto', 
          'Unidad Medida Proyecto', 
          'Fecha Fin Proyecto', 
          'Fecha Inicio Proyecto', 
          'Cotización Proveedor Proyecto', 
          'Cantidad Verificada Proyecto', 
          'Ubicación Proyecto', 
...
private addHeader(doc: jsPDF, fechaInicio: string, fechaFinal: string, margin: any) {
  ...
  doc.text('Reporte de Insumo a Proyecto', doc.internal.pageSize.width / 2, 50, { align: 'center' });
