...
@Component({
  ...
  templateUrl: './ReporteEmpleadoPorEstado.component.html',
  styleUrls: ['./ReporteEmpleadoPorEstado.component.scss'],
...
generateReport() {
  ...
  this.reporte.GetReporteEmpleadoPorEstado(estado).subscribe(
    (data: any[]) => {
      this.loading = false;
      ...
      autoTable(doc, {
        startY: 60,
        head: [['No.', 'DNI', 'Nombre Completo', 'Teléfono', 'Email', 'Salario', 'Ciudad Estado', 'Cargo', 'Banco', 'No. Bancario']],
        body: data
          .filter(item => item) // Filtrar elementos vacíos
          .map((item, index) => [
            index + 1, // Número de fila
            item.dNI || '',
            item.empl_NombreCompleto || '',
            item.telefono || '',
            item.email || '',
            (item.salario || ''), // Asegurarse de que el salario esté correctamente formateado
...
private addHeader(doc: jsPDF, fechaInicio: string, fechaFinal: string, margin: any) {
  ...
  doc.text('Reporte de Colaboradores', doc.internal.pageSize.width / 2, 50, { align: 'center' });
