...
@Component({
  selector: 'app-proveedoresmascomprados',
  templateUrl: './proveedoresmascomprados.component.html',
  styleUrl: './proveedoresmascomprados.component.scss'
})
export class ProveedoresmascompradosComponent implements OnInit {
  pieOptions: any;
  barData: any;
  ...
  ListadoFiltrado() {
    ...
    this.service.ProveedoreMasComprados(formattedFechaInicio, formattedFechaFinal).subscribe(
      (data: any) => {
        ...
        const labels = data.map((item: any) => item.prov_Descripcion);
        const datos = data.map((item: any) => item.numeroDeCotizaciones);

        // Usa los mismos colores definidos en el CSS
