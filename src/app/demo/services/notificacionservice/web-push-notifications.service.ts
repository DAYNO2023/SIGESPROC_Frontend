...
export class WebPushNotificationsService {
  ...
  private enableServiceWorker(): void {
    try {
      //verifica si el permiso no es granted
      if (Notification.permission != 'granted') {
        // this.requestSubscription(); //entra a requestSubscription
      } else {
  ...
  */
  private sendSubcriptionObject(subscription): void {
    const apiUrl = `${environment.apiRoot}/api/Notifications/installations`;
